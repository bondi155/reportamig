...
async function armaResultVert(pAnio, pMes, pAnioAnt, pArrCia, pArrEnc, pDetalles, pProcEnc = 1) {
  const resultados = [];
  let valParam = [];
  let detalle;
  let lineaSal;
  ...
  for (let i = 0; i < pDetalles.length; i++) {
    ...
    for (let idxCia = 0; idxCia < pArrCia.length; idxCia++) {
      ...
      if (detalle.val_fijo !== null) {
        ...
        if (detalle.val_fijo.startsWith('=')) {
          const expression = detalle.val_fijo.slice(1);
          const parsedExpression = reempParam(expression, idCia, pAnio, pMes, pAnioAnt, valParam);
        ...
        sqlDinamica = reempParam(detalle.campo_orig, idCia, pAnio, pMes, pAnioAnt, valParam);
        ...
        if (tablaOrig === null) {
          ...
          whereCond = reempParam(detalle.where_cond, idCia, pAnio, pMes, pAnioAnt, valParam);
          consultaFinal = `SELECT ${sqlDinamica} AS ${alias} FROM ${tablaOrig} WHERE ${whereCond}`; //
        }
        try {
          const [resultadoConsulta] = await pool.promise().query(consultaFinal);
  ...
  try {
    ...
    if(resultadoEnc.tipo_proc=="V") {
      for (let tabElem of resultadoTab) {
        ...
        const consultas =await armaResultVert(pAnio, pMes, pAnioAnt, companias, encabezadoCompleto, detallesD);
...
// Armado de excel 
async function reporteMapExcel(datosTotal, anio, mes, nombreArchivo, tipoArchivo) {
...
console.log('TIPO DE ARCHIVO!!!!!!', tipoArchivo)
 ...
 // Asumiendo que la pestaña es la primera en datosTotal
const pestaña = datosTotal[0];
const sheet = workbook.getWorksheet(pestaña.tab_nombre);

// Colocar encabezados desde C10 hacia abajo
let encabezadoRowStart = 10;
pestaña.encabezado[0].slice(1).forEach((encabezadoItem, index) => {
  // Omitimos el primer elemento porque está vacío y no lo queremos en el Excel
  const cellRef = 'C' + (encabezadoRowStart + index);
  const valorEncabezado = Object.values(encabezadoItem)[0];
  sheet.getCell(cellRef).value = valorEncabezado;
});

// Colocar los nombres de las empresas desde G8 hacia la derecha
let nombreEmpresaRow = 8;
let dataRowStart = 10; // La fila donde comienzan los datos debajo de los nombres de las empresas
let dataColumnStart = 'G'.charCodeAt(0); // La columna donde comienza "Empresa 1"

// Insertamos los nombres de las empresas
pestaña.detalle[0].slice(2).forEach((nombreEmpresa, index) => {
  // Comenzamos desde la tercera empresa para omitir la primera columna vacía y "Total"
  const cellRef = String.fromCharCode(dataColumnStart + index) + nombreEmpresaRow;
  const nombre = Object.values(nombreEmpresa)[0];
  sheet.getCell(cellRef).value = nombre;
});

// Ahora insertamos los datos debajo de los nombres de las empresas
pestaña.detalle.slice(1).forEach((detalleFila, filaIndex) => {
  let dataRow = dataRowStart + filaIndex;
  detalleFila.slice(2).forEach((data, dataIndex) => {
    // Comenzamos desde la tercera columna de datos para alinearnos con los nombres de las empresas
    let dataColumn = String.fromCharCode(dataColumnStart + dataIndex);
    const cellRef = dataColumn + dataRow;
...
exports.generarYDescargarExcel = async (req, res) => {
  ...
  // console.log('Detalle de la primera pestaña:', JSON.stringify(datosTotal[0].detalle, null, 2));
    ...
    if(datosTotal) {
      const rutaSalida = await reporteMapExcel(datosTotal, anio, mes, nombreArchivo, mapeoEnc.tipo_proc);
